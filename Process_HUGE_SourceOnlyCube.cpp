...
int main(int argc, char* argv[]){
  ...
  long int * flag_vals, o, obj, NOobj;
  int * xyz_order;
  size_t * data_metric;
  ...
  int SFmethod = 1, NOtrue, NOobj_positive;
  float label_y_shift = 0.0, progress, intens_thresh_min = -1E10, intens_thresh_max = 1E10;

  // variables unique to this code, used to specify flag values
  int flag_val_em = -1;
  ...
  if((argc == 2) && (output_code != "-h")){ cout << "WARNING: Incorrect arguments! Exiting.\nEnter Process_HUGE_SourceOnlyCube_v2 on the command line to see the command line input.\n"; return 1; }
  ...
  for(j = 0; j < temp_chunk_y_size; ++j){
	...
	if(detections[obj_batch][(obj - (obj_batch * obj_limit))].ShowVoxels() >= 1){ ++o; }
  ...
  outputfile << "# Catalogue produced by Process_HUGE_SourceOnlyCube.cpp.\n# " << endl;
