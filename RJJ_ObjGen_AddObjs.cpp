...
int AddObjsToChunk(int * flag_vals, object_props ** detections, int NOobj, int obj_limit, int chunk_x_start, int chunk_y_start, int chunk_z_start, int chunk_x_size, int chunk_y_size, int chunk_z_size, int * check_obj_ids, int * data_metric, int * xyz_order){

  float progress;
  int obj, obj_batch, NO_check_obj_ids, x, y, g, f;
  ...
  if(NOobj > 0){
	...
	std::cout << "WARNING!!! Index equals/exceeds size of check_obj_ids array in memory; truncating index to " << (SIZE_CHECK_OBJ_IDS - 1) << std::endl;
	NO_check_obj_ids = (int) SIZE_CHECK_OBJ_IDS - 1;

      }
      check_obj_ids[NO_check_obj_ids] = obj;
      NO_check_obj_ids++;
