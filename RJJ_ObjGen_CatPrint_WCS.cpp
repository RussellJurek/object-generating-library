...
int CreateCatalogue_WCS(std::fstream& output_file, object_props ** detections, int NOobj, int obj_limit, int cat_mode, fitsfile * fits_input){
  ...
  status = 0;
  //std::cout << "Extracting wcs from fits_input . . . " << std::endl;
  fits_hdr2str(fits_input,1,NULL,0,&header,&nkeys,&status);
  //std::cout << nkeys << " " << status << ": " << header << std::endl;
  //std::cout << "a " << std::endl;
  wcspih(header,nkeys,WCSHDR_all,2,&nreject,&nwcs,&wcs);
  //std::cout << nreject << " " << nwcs << std::endl;
  //std::cout << "b " << std::endl;
  ...
  //std::cout << "d " << std::endl;
  wcsset(wcs);
  //std::cout << "Finished constructing wcs." << std::endl;
