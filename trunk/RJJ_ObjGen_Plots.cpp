...
void CreateObjPlots(int m, int plot_mode, std::string output_code, int * xyz_order, int NOx, int NOy, int NOf, object_props ** detections, int NOobj, int obj_limit, int obj_batch_limit, int ctype3, float crpix3, float crval3, float cdelt3, float restfreq){
  ...
  if((m > 1) && (plot_mode > 0)){
    ...
    if((plot_mode == 1) || (plot_mode == 3)){
      ...
      // de-bugging, create array of moment 0 map values
      std::cout << "Creating moment 0 map of total intensity . . . " << std::endl;
      plot_max = CreateMoment0Map(plot_array,NOobj,detections,NOx,NOy,obj_limit,obj_batch_limit);
	...
	plot_max = CreateRAPVPlot(plot_array,NOobj,detections,NOx,NOf,obj_limit,obj_batch_limit);
	...
	plot_max = CreateDecPVPlot(plot_array,NOobj,detections,NOy,NOf,obj_limit,obj_batch_limit);
